...
async function readArticleBehavior(page, durationMs, debug) {
  ...
  while (Date.now() - startTime < durationMs) {
    ...
    if (action < 50) {
      ...
      actions++;
      if (debug) console.log(`  [Action ${actions}] Scrolled`);
      ...
      // Tab switch simulation (blur/focus)
      await page.evaluate(() => window.dispatchEvent(new Event('blur')));
      await sleep(rand(1000, 3000));
      await page.evaluate(() => window.dispatchEvent(new Event('focus')));
      if (debug) console.log(`  [Action ${actions}] Tab switch simulation`);
    }
...
/* ---------- Blog Interaction ---------- */
async function interactWithBlog(page, minStay, maxStay, debug) {
  // Determine session type: bouncer (30s-2min) vs reader (3-10min)
  const isEngaged = Math.random() < 0.7; // 70% are engaged readers
  const stayTime = isEngaged ? rand(180000, maxStay) : rand(30000, 120000);
  
  if (debug) console.log(`  Session type: ${isEngaged ? 'ENGAGED READER' : 'QUICK SCAN'}, duration: ${Math.round(stayTime/1000)}s`);
  
  // Initial page settle
  await sleep(rand(2000, 4000));
  
  // Check if we need to handle any popup/modal
  try {
    await page.evaluate(() => {
  ...
  // Read homepage
  await readArticleBehavior(page, Math.min(stayTime * 0.4, 120000), debug);
  
  // Click a post if engaged
  if (isEngaged || Math.random() < 0.4) {
    const postClicked = await clickRandomPost(page, debug);
    
    if (postClicked) {
      const postReadTime = stayTime * 0.6;
      await sleep(rand(2000, 5000));
      await readArticleBehavior(page, postReadTime, debug);
      
      // Sometimes go back to homepage
      if (Math.random() < 0.3) {
        await page.goBack();
        await sleep(rand(1000, 3000));
        await readArticleBehavior(page, rand(10000, 30000), debug);
      }
    }
  }
  
  // Natural exit (scroll to top first sometimes)
  if (Math.random() < 0.4) {
...
async function clickRandomPost(page, debug) {
  try {
    const postInfo = await page.evaluate(() => {
      // Common WordPress/blog selectors
      const selectors = [
      ...
      const link = links[Math.floor(Math.random() * Math.min(links.length, 5))]; // Prefer top posts
      const rect = link.getBoundingClientRect();
...
(async () => {
  ...
  while (!stop && (cfg.forever || run < cfg.runs)) {
    ...
    try {
      const page = await setupStealthPage(browser, device);
      
      // FIXED: Use new X.com navigation function
      ...
      await interactWithBlog(page, cfg.minStay, cfg.maxStay, cfg.debug);
